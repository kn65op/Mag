...
W~przypadku sygnałów dyskretnych problemem staje się miejsce występowania miejsc, w którym pierwszy warunek z \ref{eq:edgeDetection} jest spełniony. Nie jest to w punktach gdzie znajdują się piksele, lecz pomiędzy nimi. Z~tego powodu zmiast porównywania do zera w~warunku pierwszy sprawdzane jest czy zmienia się znak pomiędzy dwoma pikselami. Dla analizowanego piksela sprawdzany są jedynie trzy sąsiednie piksele oznaczone na rys. \ref{fig:comparePixels}. 

Drugim problemem są błedy obliczeń spowodowane dyskretyzacją sygnałów i~ograniczoną możliwością reprezentacji liczb w~komputerach. Z~tego powodu piksele, które powinny mieć wartość równą zeru przyjmują wartości bliskie zeru. Dodatkowo mogą być różnych znaków, co może powodować błędy podczas wykrywania punktów, w~których druga pochodna przyjmuje wartość zerową. Z~tego powodu zdecydowano, że otrzymane liczby bliskie zeru (wartość bezwzględna poniżej $ 10^{-5} $) będą miały przypiywaną wartość zero.
...
Sposób postępowania jest analogiczny jak dla wykrywania krawędzi. Liczone są odpowiednie obrazu odpowiadające warunkom \ref{eq:ridgeDetection} a~następnie są one sprawdzane.
...
Do wyznacznie narożników wykorzystywany jest współczynnik krzywizny, który jest wyznaczany zgodnie ze wzorem \ref{eq:cornerDetection}.
